...
const ScreenCopy = ({ eyebrow, title, body, solves }) => (
  <div style={{ flex: 1, minWidth: 0 }}>
    <span className="font-mono uppercase" style={{ fontSize: 11, letterSpacing: '.2em', color: '#caa066' }}>{eyebrow}</span>
    <h4 className="font-serif font-semibold" style={{ margin: '10px 0 10px', fontSize: 'clamp(22px,2.8vw,30px)', color: '#f3e8d3', lineHeight: 1.15 }}>{title}</h4>
    ...
    <div style={{ display: 'flex', gap: 12, paddingTop: 14, borderTop: '1px solid rgba(202,160,102,.14)' }}>
      <span className="font-mono uppercase flex-shrink-0" style={{ fontSize: 10, letterSpacing: '.1em', color: '#caa066', width: 58, paddingTop: 2 }}>Solves</span>
    ...
    >
      <div className="max-w-[1140px] mx-auto">
        <Reveal className="flex items-center gap-[14px] mb-[22px]">
          <span className="font-serif" style={{ fontSize: 13, letterSpacing: '.3em', color: '#caa066' }}>陸 Chapter Six</span>
        ...
        <Reveal as="h3" className="font-serif font-semibold" style={{ margin: '0 0 clamp(40px,5vw,56px)', fontSize: 'clamp(22px,2.8vw,32px)', color: '#f3e8d3' }}>
          The surfaces where work happens
        </Reveal>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(56px,8vw,104px)' }}>
          {/* Screen 1: Filtering */}
          <div className="flex flex-col lg:flex-row items-center gap-[clamp(28px,5vw,64px)]">
            <Reveal style={{ flex: '1.5', minWidth: 0, position: 'relative' }}>
              <MockupGlow>
                  ...
                  <div style={{ padding: 'clamp(16px,2.2vw,22px)' }}>
                    ...
                    <div style={{ border: '1px solid rgba(202,160,102,.16)', borderRadius: 8, overflow: 'hidden' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr .8fr .8fr .8fr .7fr', background: 'rgba(202,160,102,.08)' }}>
                        {['Line', 'Contract', 'POL', 'POD', 'Rate'].map((h) => (
                          <span key={h} style={{ padding: '8px 10px', fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: '.06em', textTransform: 'uppercase', color: '#caa066' }}>{h}</span>
                        ))}
                      </div>
          ...
          {/* Screen 2: Surcharges */}
          <div className="flex flex-col lg:flex-row-reverse items-center gap-[clamp(28px,5vw,64px)]">
            <Reveal style={{ flex: '1.5', minWidth: 0, position: 'relative' }}>
              <MockupGlow>
                  ...
                  <div style={{ padding: 'clamp(20px,2.6vw,28px)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
                        ...
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: "'Shippori Mincho',serif", fontSize: 18, fontWeight: 600, color: '#ecdcbb', background: '#1d1610', border: '1.5px solid #caa066', borderRadius: 8, padding: '5px 14px' }}>300<span aria-hidden="true" style={{ width: 1, height: 16, background: '#caa066', animation: 'pglow 1.1s steps(1) infinite' }} /></span>
                      </div>
                ...
                <MockupShell path="pot / monitoring">
                  <div style={{ padding: 'clamp(16px,2.2vw,22px)' }}>
                    <div style={{ border: '1px solid rgba(202,160,102,.16)', borderRadius: 8, overflow: 'hidden' }}>
                      ...
                      {[
                        { status: 'Published', statusStyle: { fontSize: 10, color: '#9bd4b4', background: 'rgba(127,199,160,.12)', border: '1px solid rgba(127,199,160,.5)', padding: '3px 10px', borderRadius: 999 } },
                        { status: 'Blocked',   statusStyle: { fontSize: 10, color: '#e6a48f', background: 'rgba(217,138,106,.12)', border: '1px solid rgba(217,138,106,.5)', padding: '3px 10px', borderRadius: 999 } },
                        { status: 'Published', statusStyle: { fontSize: 10, color: '#9bd4b4', background: 'rgba(127,199,160,.12)', border: '1px solid rgba(127,199,160,.5)', padding: '3px 10px', borderRadius: 999 } },
                      ].map((row, ri) => (
                        <div key={ri} style={{ display: 'grid', gridTemplateColumns: '.9fr 1.2fr .7fr 1fr', alignItems: 'center', borderTop: '1px solid rgba(202,160,102,.1)' }}>
                          ...
                          <span style={{ padding: '7px 10px', textAlign: 'right' }}><span style={row.statusStyle}>{row.status}</span></span>
                        </div>
                      ))}
                    </div>
                  </div>
