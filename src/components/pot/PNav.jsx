...
import { sections } from '../../data/pot';
import useActiveSection from '../../hooks/useActiveSection';
import useScrollProgress from '../../hooks/useScrollProgress';
import usePrefersReducedMotion from '../../hooks/usePrefersReducedMotion';
import { scrollToId } from '../../utils/scroll';

const SECTION_IDS = ['p-hero', ...sections.map((s) => s.id)];
    ...
    >
      <div className="flex items-center justify-between gap-[18px] px-[clamp(18px,4vw,42px)] py-[14px]">
        ...
        <div className="flex items-center gap-[11px]">
          ...
          <span className="font-serif text-base font-semibold" style={{ letterSpacing: '.18em', color: '#f3e8d3' }}>
            POT
          </span>
        </div>
          ...
          {sections.map((s) => {
            ...
            return (
              <a
                ...
                style={{ letterSpacing: '.02em', padding: '6px 11px', color: on ? '#15100c' : '#bda783', background: on ? '#caa066' : 'transparent' }}
              >
                {s.label}
              </a>
            );
