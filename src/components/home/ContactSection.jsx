...
export default function ContactSection() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!FORMSPREE_ID) { setStatus('sent'); return; } // fallback: show success without sending
    setStatus('sending');
    try {
  ...
  return (
    <section
      ...
      style={{ padding: 'clamp(60px,8vw,110px) 0 0', background: 'linear-gradient(180deg,#f7e9cf 0%,#f5dfb6 34%,#eecb95 64%,#e6b87a 100%)' }}
    >
      {/* sunrise */}
      <div
        aria-hidden="true"
      ...
      <div className="relative z-3 max-w-[1000px] mx-auto px-[clamp(22px,6vw,64px)] text-center">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-[clamp(24px,5vw,56px)] text-left" style={{ marginTop: 'clamp(28px,4vw,52px)' }}>
          {/* left: links */}
          ...
          <Reveal delay={120}>
            {status === 'sent' ? (
              ...
              >
                ...
                {status === 'error' && (
                  <p className="m-0 text-[13px] text-center" style={{ color: '#a8552f' }}>
                ...
                <button
                  ...
                  className="rounded-full text-[14.5px] border-none transition-colors duration-400 hover:bg-[#3a342b] disabled:opacity-60"
      ...
      {/* summit silhouette */}
      <div className="relative" style={{ height: 'clamp(80px,10vw,150px)', marginTop: 'clamp(24px,4vw,52px)' }}>
        <ParallaxLayer
          as="svg"
          anchor="contact"
          speed={0.06}
